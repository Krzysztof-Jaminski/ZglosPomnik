...
class TreesService {
  ...
  // Głosowanie na drzewo - PUT /api/Trees/{id}/vote (now only takes id, no vote type)
  async voteOnTree(treeId: string): Promise<{ like: number; dislike: number }> {
    try {
      const token = authService.getToken();
      if (!token) {
        throw new Error('No authentication token');
  ...
  // Usuwanie głosu na drzewo - DELETE /api/Trees/{id}/vote (returns vote count)
  ...
  async submitTreeReport(treeData: ApiTreeSubmission, photos: File[], mapScreenshot?: File | null): Promise<any> {
    try {
      ...
      // Add location fields (empty strings - backend will populate)
      formData.append('location.address', treeData.location.address || '');
      formData.append('location.plotNumber', treeData.location.plotNumber || '');
      formData.append('location.district', treeData.location.district || '');
      formData.append('location.province', treeData.location.province || '');
      formData.append('location.county', treeData.location.county || '');
      formData.append('location.commune', treeData.location.commune || '');
      ...
      if (treeData.legend) {
        formData.append('legend', treeData.legend);
      }
