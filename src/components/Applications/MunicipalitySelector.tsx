...
export const MunicipalitySelector: React.FC<MunicipalitySelectorProps> = ({
  ...
  const filteredMunicipalities = useMemo(() => {
    ...
    return municipalities.filter(municipality => 
      ...
      municipality.city.toLowerCase().includes(query) ||
      municipality.address.toLowerCase().includes(query)
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="max-h-[75vh] overflow-y-auto space-y-2 pr-2 py-2">
        {filteredMunicipalities.map(municipality => (
          <motion.div
            ...
            className={`bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg cursor-pointer transition-all p-3 ${
              selectedMunicipality?.id === municipality.id ? 'bg-green-50/50 dark:bg-green-900/20' : 'hover:shadow-xl hover:bg-white/90 dark:hover:bg-gray-800/90'
            }`}
          >
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                ...
                <div className="space-y-0.5 text-xs text-gray-600 dark:text-gray-400">
                  <p className="truncate">Email: {municipality.email}</p>
                  <p className="truncate">Telefon: {municipality.phone}</p>
                  <p className="truncate">Adres: {municipality.address}, {municipality.city}</p>
                </div>
              </div>
        ...
        {filteredMunicipalities.length === 0 && searchQuery && (
